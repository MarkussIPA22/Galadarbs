...
export default function EditWorkout({ auth, workout, exercises, favoriteExercises }) {
  ...
  const filteredExercises = exercises.filter((exercise) => {
    const matchesGroup = data.muscle_groups.includes(exercise.muscle_group.toLowerCase());
    const matchesFavorites = showFavorites && favoriteExercises.includes(exercise.id);
    const matchesFilter = filter === '' || exercise.name.toLowerCase().includes(filter.toLowerCase());
    return matchesGroup && (matchesFavorites || matchesFilter);
  });
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="mt-4">
              <input
                ...
                className="w-full p-2 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200"
              />
            </div>

            <div className="mt-6">
              <h3 className="font-semibold mb-2">{t('select_exercises')}</h3>
