...
<input
  ...
  onChange={(e) => setNewExercise({ ...newExercise, name_lv: e.target.value })}
  className="p-2 rounded-lg bg-gray-700 text-white"
/>
...
<input
  ...
  onChange={(e) => setNewExercise({ ...newExercise, muscle_group_lv: e.target.value })}
  className="p-2 rounded-lg bg-gray-700 text-white"
/>
...
<textarea
  ...
  className="col-span-2 p-2 rounded-lg bg-gray-700 text-white"
/>

<textarea
  placeholder={t('description_lv')}
  value={newExercise.description_lv}
  onChange={(e) => setNewExercise({ ...newExercise, description_lv: e.target.value })}
  className="col-span-2 p-2 rounded-lg bg-gray-700 text-white"
/>
...
<button
  type="submit"
  disabled={processing}
  className="col-span-2 bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-xl"
>
  {t('save_exercise')}
</button>
          ...
          <div className="mb-6">
            ...
            >
              ...
              {[...new Set(exercises.map((ex) => i18n.language === 'lv' ? ex.muscle_group_lv : ex.muscle_group))].map(
                (group, idx) => (
...
<div className="space-y-4">
  ...
  {filteredExercises.map((exercise) => (
    <div
      key={exercise.id}
      className="bg-gray-800 p-4 rounded-lg shadow-md flex flex-col sm:flex-row sm:items-center sm:justify-between"
    >
      <div className="flex-1">
        <h3 className="text-lg font-semibold">
          {i18n.language === 'lv' ? exercise.name_lv : exercise.name}
        </h3>
        <p className="text-sm text-gray-400">
          {i18n.language === 'lv' ? exercise.muscle_group_lv : exercise.muscle_group}
        </p>
        <p className="text-gray-300 mt-2">
          {i18n.language === 'lv' ? exercise.description_lv : exercise.description}
        </p>
        {exercise.image_path && (
          <img
            src={exercise.image_path}
            alt={i18n.language === 'lv' ? exercise.name_lv : exercise.name}
      ...
      <button
        ...
        className="mt-4 sm:mt-0 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg"
      >
        {t('delete')}
      </button>
    </div>
