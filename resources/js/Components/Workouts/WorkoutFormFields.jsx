...
export default function WorkoutFormFields({
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="group">
        <label className="flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-200 mb-3 uppercase tracking-wider">
          {t('description')} <span className="text-xs text-gray-500 dark:text-gray-400">({t('optional')})</span>
        </label>
        <textarea
      ...
      {!editMode && (
        <div>
          ...
          {errors.muscle_groups && <p className="text-red-500 mt-2">{errors.muscle_groups}</p>}
        </div>
      )}
    </div>
    </div>
